#!/usr/bin/env python3
"""
Drone Tracking Pipeline - Multi-threaded

Architecture (single AirSim client to avoid conflicts):

  Main Thread (AirSim calls)  -->  Queue  -->  Tracking Thread  -->  Queue  -->  Main (visualization)
       |
       +-- Also sends drone movement via moveOnPathAsync

  When YOLO+PointNet are ready, a DetectionThread is inserted between
  Main and Tracking, processing the RGB image + depth into 3D detections.
  For now, ground truth from simGetDetections is used directly.

Tracking thread: SORT 3D tracker with Kalman filters.
"""
...
from sort_tracker_3d import Sort3D
