#!/usr/bin/env python3
"""FASE 2 — Processa uma sessão gravada (record_session.py) com o pipeline COMPLETO:
YOLO@1280 → frustum PointNet → fusão YOLO×PN → EKF multi-target tracker.

Sem pressão de tempo (offline) → roda no full e grava vídeo em TEMPO REAL (fps da gravação),
com tracks estáveis (IDs, trajetória, predição entre frames).
"""
...
from ekf_3d_global import EKFParams
import inference_pipeline as P   # depth_to_pointcloud_cv, cv_to_frd, frd_to_spherical, draw_overlay, project_global_to_pixel
...
def main():
    ...
    yolo = YOLO(args.yolo)
    det = FrustumPN2Detector(checkpoint_path=args.pointnet, cls_threshold=0.0)  # threshold via fusão
    ...
    for fr in frames:
        ...
        # associa bbox ao track (mesma lógica do pipeline)
        track_bboxes = {}
