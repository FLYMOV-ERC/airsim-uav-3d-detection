# AirSim 3D environments — how to install and use them
...
>
> **Status.** Operational notes on the AirSim environments. The canonical
> collector is now `dataset_generation/collect_airsim.py`; the `collect-dataset.py`
> this document refers to is archived at
> `archive/dataset_generators/collect_dataset.py`.
...
## How to install new environments

### Option 1: pre-built environments (easiest)
...
## Alternatives without downloading new environments

### Inside the same Blocks environment you can:
...
- **Cenários de MOVIMENTO**: sim, já funcionam (formações, padrões)
- **Mudanças de CLIMA/HORÁRIO**: sim, já funcionam
- **Ambientes 3D DIFERENTES**: não, precisa baixar/instalar separadamente
- **Trocar ambientes**: atenção, precisa reiniciar o simulador