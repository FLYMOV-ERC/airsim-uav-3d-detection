# Additional voxel experiments — variants of the single-stage detector (PointPillars)
...
> there, AMOTA 0.69 against 0.68), and why has not been established. Note that
> the chapter's voxel figures are *also* full-campaign: Section 7.4.3 reports the
> 12-sequence held-out recomputation in its own text but leaves Table 7.4 showing
> the full-campaign numbers, flagged there as mildly optimistic. So the gap
> between this file and the chapter is not the held-out correction. Where the two
...
## Comparison against the frustum (paradigms side by side)
