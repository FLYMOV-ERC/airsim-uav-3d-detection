# Overall comparison — ALL methods (frustum + voxel + fusion)
...
>
> **Status: superseded, and the difference is untraced.** These figures do not
> match Chapter 7's Table 7.4 — PointPillars-PFN RMSE 0.78 m here against 0.87 m
> there, T-Net 1.03 m against 1.05 m, best AMOTA 0.83 against 0.81 — and most
> other rows differ slightly as well. This file predates the dissertation's final
> numbers; where the two disagree, the dissertation is the reported result.
>
> **Why they differ has not been established, and no explanation is given here.**
> It is not the held-out recomputation of Section 7.4.3: that concerns only the
> voxel rows, the frustum rows differ here too, and Section 7.4.3 states that the
> voxel figures printed in the chapter are the full-campaign ones.
...
## Leaders per metric
- **Localização 3D (RMSE):** melhor, **PFN 0.78 m** › Voxel-3D 0.85 m › T-Net 1.03 m › Fusão 1.09 m › PP base 1.12 m.
  *O voxel-PFN passou a ser o melhor localizador de todos — supera o melhor frustum (T-Net) em 24%.*
- **Recall (det-R):** melhores, **Voxel-3D/PFN ~0.80** » todos os frustum ≤0.49 (teto imposto pelo YOLO).
- **Detecção F1:** melhores, **PFN/Voxel-3D 0.84** › PP base 0.80 › Fusão 0.75 › T-Net 0.60.
- **Precisão (det-P):** empate no topo — **PFN/Voxel-3D/T-Net/depth-band ~0.91–0.92**.
- **AMOTA:** melhor, **T-Net 0.83** › depth-band 0.82 › F-ConvNet 0.81 › Fusão 0.75 › PFN/Voxel-3D 0.69.
- **ID-switches:** frustum « voxel (frustum ~50; voxel-PFN 429, já bem melhor que o PP base ~666).

## Reading (what changed with the voxel experiments)
...
## Late fusion T-Net ⊕ PointPillars-PFN (two anchorings)
