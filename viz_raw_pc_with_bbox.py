#!/usr/bin/env python3
"""Gera PLYs do RAW PC (não normalizado) dos dataset_v4 + bbox 3D label.

Mostra:
  - PC raw (frame CV: x=right, y=down, z=fwd) cinza
  - Bbox 3D label SEM fix (airsim_to_cv direto) — VERMELHA
  - Bbox 3D label COM fix pitch+offset — VERDE
  - Esfera no center label

Útil pra debugar offset da bbox vs PC real.
"""
import json
import numpy as np
...
def main():
    ...
    for env, ds, frame, lbls in frames_to_check:
        ...
        # PC já está em frame CV (camera-tilted)
        # Sub-sample pra agilizar viz
        if len(pc) > 50000:
            idx = np.random.choice(len(pc), 50000, replace=False)
            pc = pc[idx]
        # Color por depth (mais perto = mais claro)
        ...
        for i, l in enumerate(lbls):
            ...
            print(f"  {env}/{frame}/{l['name']}: dist={l['distance_m']:.1f}m  "
                  ...
                  f"v1 (sem fix) center_cv={center_v1.round(2).tolist()}  "
                  f"v2 (com fix) center_cv={center_v2.round(2).tolist()}  "
    ...
    print("Legenda: VERMELHO=sem fix (rel_pose direto), VERDE=com fix pitch+offset")
