#!/usr/bin/env python3
"""Métricas com o tracker do PIPELINE FINAL: SORT (2D-IoU) faz só a associação/ID,
e cada track recebe um EKF 3D (o EKFTracker3D dos Cap.5/6, medição range-bearing) que
estima/filtra a posição 3D. As posições reportadas (e o RMSE 3D) são as do EKF, não as
detecções cruas. Mesma estrutura de métricas do metrics_from_dump.py (CLEAR-MOT + AMOTA)."""
...
def run_tracker(frames, xpof, fuse_w, thr, max_age=15, min_hits=3, max_dist=90):
    ...
    for fr in frames:
        ...
        for t in tracks:
            ...
            if t.time_since_update==0 and xp is not None:   # medição fresca -> update
