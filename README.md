...
dataset_generation/
  ...
  record_campaign.sh      the campaign driver: 20 sequences per environment, resumable
...
# the 60-sequence evaluation campaign, once per environment (20 sequences each)
$ bash dataset_generation/record_campaign.sh nh
$ bash dataset_generation/record_campaign.sh city
$ bash dataset_generation/record_campaign.sh coast
```
...
|---|---|---|
| **Table 7.1** | summary of the data (synthetic ≈ 2,500 frames; collected 3 × 1,000; campaign 60 sequences) | `dataset_generation/synthetic/compose.py`, `dataset_generation/collect_airsim.py`, `dataset_generation/record_campaign.sh` → `dataset_generation/record_campaign.py`; counts verifiable with `evaluation/qa/analyze_dataset.py` |
...
| `localization_over_time` | 3D error over normalized sequence time, and its CDF per environment; the statistics of Section 7.5.4 | `evaluation/temporal_error.py` |
...
which writes `<outdir>/Cap4` and `<outdir>/Cap7`. The temporal analysis is a
separate script, because it recomputes its numbers from the detection dumps:

```console
$ python evaluation/temporal_error.py --data-root /path/to/data --dumps det_dumps_best --outdir /path/to/dissertation
```

It runs the exact tracker of `evaluation/metrics_ekf.py` at the reported operating
point and writes `<outdir>/Cap7/localization_over_time.pdf` plus a JSON of the
statistics: 5,359 matched pairs over 60 sequences, Spearman ρ = −0.10,
Kruskal–Wallis H = 130, Mann–Whitney p = 0.53 (first five frames against the rest).

### Figures this script does not produce

Six of the chapter's figures are **not** generated by any script in this
repository:

`frustum_pipeline_teaser`, `depthband_diagram`, `pointnet_topology`,
`approaches_diagram`, `pipeline_live`, `pipeline_frames`.

The first four are diagrams and appear to have been drawn by hand.
`localization_over_time`, previously listed here, is produced by
`evaluation/temporal_error.py` (see above).

---

## The evaluation campaign: `record_campaign.sh`
...
4.6 states that the whole 60-sequence campaign ran through it. It is
`dataset_generation/record_campaign.sh`. It was absent from the source tree this
repository was first reorganised from and was recovered afterwards from the
machine the campaign ran on; its list of 20 configurations per environment (range
regime, weather, hour, seed, target count) is the one actually used, kept verbatim.
Only the paths, the simulator host (now read from `configs/default.yaml`, like the
Python scripts) and the comments were changed.

It records each sequence with `dataset_generation/record_campaign.py` in a separate
process, so a WSL crash costs one sequence rather than the campaign, and it is
resumable: a sequence with more than 44 non-empty frames and a parseable
`meta.json` is skipped, anything else is re-recorded.
