# Resultados — detecção e tracking 3D de drones (AirSim, 60 sequências)
...
## Métricas (padrão da literatura 3D-MOT: nuScenes/AB3DMOT + detecção + localização)
...
## Interpretação — os resultados fazem sentido?
...
## Ablação — Baseline (frustum v4, sem depth-band, EKF) vs Proposto (depth-band + SORT)
