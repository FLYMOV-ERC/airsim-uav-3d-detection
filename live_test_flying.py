#!/usr/bin/env python3
"""Teste live com drones GARANTIDAMENTE voando + detecção YOLO→Frustum PointNet.

- Decola todos, sobe ego pra altitude segura.
- Drones em órbita lenta À FRENTE do ego, re-comandada continuamente (thread)
  pra nunca caírem (resolve o problema de cair no terreno do Coastline).
- VisQuad 4x (DroneVisualizer corrigido) pra YOLO ver o mesh treinado.
- Loop: captura RGB+depth, YOLO@1280, frustum PointNet por bbox, overlay + vídeo.
"""
...
class FlightThread(threading.Thread):
    """Voo CINEMÁTICO: teleporta cada drone numa órbita lenta + SEGURA o ego no ar
    (vehicle frame) a ~15Hz. Sem física/takeoff → nada cai, nada trava em terreno."""
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            # EGO em MOVIMENTO (plataforma móvel) — oscilação 3D lenta + leve yaw,
            # amplitude pequena p/ manter drones no FOV. Testa o transform global do tracker.
            ...
            for i, d in enumerate(self.drones):
                ...
                yaw = t + i*2.1 + math.pi/2  # nariz na direção do movimento
                try:
...
def main():
    ...
    ap.add_argument("--fuse_w", type=float, default=0.5,
                    help="Peso do YOLO na fusão: score=w*yolo+(1-w)*pn. Aceita se score>=fuse_thr.")
    ap.add_argument("--fuse_thr", type=float, default=0.5,
                    help="Threshold no score fundido YOLO×PN.")
    ap.add_argument("--imgsz", type=int, default=960, help="imgsz do YOLO (menor=mais fps)")
    ...
    # Ego: teleporta pra altitude segura (vehicle z=-20 → global -25). Sem takeoff.
    EGO_Z = -20.0
    c.simSetVehiclePose(airsim.Pose(airsim.Vector3r(0, 0, EGO_Z), _yaw_quat(0)), True, "Ego")
    time.sleep(0.5)

    # Centros de órbita À FRENTE do ego (global). Câmera tem pitch -15° (olha p/ baixo),
    # então os drones precisam ficar ABAIXO do ego (~dist*tan15°) p/ centralizar no quadro.
    ego_g = np.array([0.0, 0.0, EGO_Z + VEHICLE_ORIGINS['Ego'][2]])  # global ego ~ (0,0,-25)
    # z global MENOS negativo = mais baixo. A 30m, 15° p/ baixo ≈ 8m abaixo do ego.
    # ESPALHADOS (esq/centro/dir, distâncias diferentes) p/ caixas não se sobreporem.
    targets_global = {
        'Drone3':    (ego_g[0]+22, ego_g[1]-14, ego_g[2]+5),   # perto, esquerda
        'Drone4':    (ego_g[0]+45, ego_g[1]+16, ego_g[2]+12),  # longe, direita
        'Intruder1': (ego_g[0]+33, ego_g[1]+1,  ego_g[2]+8),   # médio, centro
    }
    centers = {}
    for d in drones:
    ...
    # Thread de voo contínuo
    stop = threading.Event()
    fly = FlightThread(args.ip, args.port, drones, centers, EGO_Z, stop, radius=6.0, w=0.25)
    fly.start()
    print(f"[Fly] drones orbitando à frente do ego. Detectando por {args.duration}s...")

    frames_buf = []  # buffer p/ gravar na fps REAL no fim (senão vídeo fica acelerado)
    ...
    try:
        while time.time() - t_start < args.duration:
            ...
            if res and res[0].boxes is not None and len(res[0].boxes) > 0:
                ...
                for i in range(len(xyxy)):
                    ...
                    # Fusão YOLO×PN: YOLO confiante compensa o PN tímido
        ...
        # grava na fps REAL → vídeo em tempo real (não acelerado)
        ...
        for f in frames_buf:
            writer.write(f)
        writer.release()
        print(f"[Video] {n_frames} frames @ {real_fps:.1f} fps (tempo real)")
