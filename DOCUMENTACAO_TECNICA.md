# Sistema de Detecção 3D de Drones em Ambientes UAM Simulados
...
## 4. Pipeline de Coleta de Dados

### 4.1 Setup AirSim

**Hardware**: AirSim roda no Windows; scripts Python no WSL/Linux. Comunicação via TCP (IP 172.19.80.1 = WSL gateway).
...
### 4.5 Configuração de posicionamento

#### Vantage points do Ego
...
#### Posicionamento dos drones (collision-aware)
...
### 4.6 Resolução de Race Conditions
...
### 4.7 Re-teleport per-frame para eliminar drift acumulado
...
### 4.8 Filtro de oclusão via depth
...
### 4.9 Tratamento de API misses (Detection API ≠ Render)
...
### 4.10 Estatísticas do dataset coletado
...
#### Densidade Point Cloud (após filtro 250m)
...
## 5. Detecção 2D: YOLO11s

### 5.1 Arquitetura

**YOLO11s** (scale=s): YOLOv11 small. ~10M parâmetros. Escolhido por:
1. Estado-da-arte 2024 (Ultralytics)
2. Compatibilidade com tracker do laboratório (`/home/ericyos/tub/ubaswarm_ws/src/tracker/`)
...
`dataset.yaml`:
```yaml
path: /home/ericyos/airsim/dataset_urban_merged/yolo
train: images/train
val: images/val
nc: 1
names: ['drone']
...
### 5.3 Hiperparâmetros de treinamento
...
## 6. Detecção 3D: Abordagens com PointNet
...
#### 6.2.4 Dataset Painted (versão inicial 88/12)
...
#### 6.2.7 Resultados Painted v2 (fix dupla-balance, só sampler)
...
### 6.3 PointNet++ com Focal Loss (Experimento A)
...
#### 6.3.2 Implementação from-scratch (PyTorch puro)
...
#### 6.3.4 Iterações de hiperparâmetros
...
#### 6.3.5 Solução: re-balanceamento agressivo do dataset
...
### 7.1 Tabela completa de todos os experimentos

| # | Modelo | Dataset | Train Task | Val Acc | Val F1 | Prec | Rec | C MSE | Status |
|---|---|---|---|---|---|---|---|---|---|
| 1 | PointNet baseline | `dataset_pointnet` (67/33) | drone vs random | **0.967** | - | - | - | 3.6 | ✅ |
| 2 | PointNet Painted v1 | `dataset_painted` (88/12) | drone vs YOLO FP | 0.499 | - | 0.99 | 0.35 | 0.003 | ⚠ bias |
| 3 | PointNet Painted v2 | `dataset_painted` (88/12) | drone vs YOLO FP | 0.651 | - | 0.96 | 0.62 | **0.003** | ✅ |
...
| 7 | **PointNet++ v4** | `dataset_painted_v2` (26/74) | drone vs YOLO FP + random | **0.775** | **0.68** | 0.58 | **0.93** | **0.003** | ✅ |
...
### 7.3 Resumo dos modelos finais para produção
...
## 8. Análise Técnica e Insights

### 8.1 Por que PointNet baseline > PointNet++ Painted em accuracy?
...
### 8.2 Por que PointPainting funciona para regression?
...
### 8.3 Iterações de balanceamento de classes
...
### 8.4 Limitações fundamentais identificadas
...
#### 8.4.4 Convenção de coordenadas AirSim `relative_pose`
...
## 10. Estrutura de Arquivos e Scripts

### 10.1 Diretório do projeto
...
### A.1 Coleta de dados (rodar 1x por ambiente)
...
### A.4 Conversão para CloudCompare
...
## Apêndice B: Métricas de Cada Run
