#!/usr/bin/env python3
"""PointPillars com PFN (Pillar Feature Network) — features de pillar APRENDIDAS.
Em vez das features hand-crafted [count, mean_down, max_fwd], cada pillar passa por um
mini-PointNet por ponto (MLP + max-pool sobre os pontos da célula) → C features aprendidas →
pseudo-imagem BEV → mesma CNN center-based (heatmap+reg). É o baseline voxel "justo/forte".
Modos: train (default) e dump (--dump)."""
...
from pointpillars_lite import XMIN,XMAX,ZMIN,ZMAX,CELL,GW,GH,gt_cv,cell_of

NCELL = GH*GW
MAXP  = 12000   # cap de pontos por frame (segurança de memória)
PC    = 64      # dim das features aprendidas do pillar
...
def collate(batch):
    ...
    for b,(p,g,hm,reg,mask) in enumerate(batch):
        pts.append(p); gidx.append(g + b*NCELL)   # índice global de pillar (batch*ncell + lin)
...
class PFNNet(nn.Module):
    def __init__(self):
        super().__init__()
        # mini-PointNet por ponto: feat aug 6D -> PC
    ...
    def forward(self, pts, gidx, B):
        ...
        cxc=XMIN+(j+0.5)*CELL; czc=ZMIN+(i+0.5)*CELL   # centro do pillar
