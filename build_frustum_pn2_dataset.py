#!/usr/bin/env python3
"""Build dataset frustum p/ PointNet++ usando bboxes YOLO @conf=0.10.

Por bbox YOLO:
  - crop frustum (pontos PC dentro do bbox 2D projetado)
  - sample fixo N=4096 (com replacement se pouco)
  - normaliza XYZ (centroide + escala)
  - match com GT 3D por IoU 2D >= 0.3
      match → positivo: center+size regression
      mismatch → false positive: cls=0, dummy regression

Adiciona negativos aleatórios (random crops fora de qualquer GT bbox).
...
def airsim_body_to_cv_camera(p_body):
    ...
    Considera camera offset + pitch -15° aplicado pelo Unreal camera render.

    p_body: AirSim relative_pose.position direto da detection ou desnormalizado da pose
    """
    # 1. Subtract camera offset (camera está em CAM_OFFSET_BODY no body frame)
    p = np.array(p_body, dtype=np.float32) - CAM_OFFSET_BODY
    # 2. Aplica rotação body → camera (R_y(-pitch) = R_y(+15°))
    a = -CAM_PITCH_RAD  # = +15°
    c, s = np.cos(a), np.sin(a)
    # body NED FRD: x=fwd, y=right, z=down. Camera tilted = mesmo eixos rotacionado pitch
    # R_y rotation: new_x = c*x + s*z; new_z = -s*x + c*z; new_y = y
    x_cam_fwd = c * p[0] + s * p[2]
    y_cam_right = p[1]
    z_cam_down = -s * p[0] + c * p[2]
    # 3. Reordena pra CV (right, down, forward)
...
def depth_band(frustum, band_m=10.0, anchor_pct=5.0):
    """Mantém só pontos perto do objeto MAIS PRÓXIMO (o drone), descartando fundo distante.
    Ancora na profundidade (z=fwd no frame CV) do percentil anchor_pct e mantém [z0, z0+band]."""
...
def crop_frustum(pc, bbox, expand_pct=0.0, band_m=0.0):
    """Crop frustum. Se expand_pct > 0, expande bbox. Se band_m > 0, corta fundo em profundidade.
    """
    x1, y1, x2, y2 = bbox
    if expand_pct > 0:
        w = x2 - x1; h = y2 - y1
...
def normalize_pc(pc_xyz):
    ...
    if NORM_MODE == "p95":
        scale = float(np.percentile(dists, 95))      # robusto a ponto distante isolado
    elif NORM_MODE == "fixed":
        scale = FIXED_SCALE                            # escala métrica fixa (como voxel)
    else:
...
def process_dataset(src_root, src_tag, out_root, yolo_subdir, rng):
    ...
    for split in ("train", "val"):
        ...
        for gt_yolo_p in sorted((gt_yolo_dir / split).glob("*.txt")):
            ...
            # === POSITIVOS/FP: 1 sample por YOLO detection ===
            for k, pred in enumerate(yolo_preds):
                ybb = tuple(pred["bbox_2d"])
                conf = pred["confidence"]

                # Expande bbox 25% pra capturar mais contexto + hélices saindo do bbox
                ...
                if best_iou >= 0.3 and best_gt:
                    # labels_3d v6 JÁ estão no frame CV camera (center, box3D em CV).
                    # Não re-transformar — usar direto.
            ...
            # === NEGATIVOS aleatórios (sem drone) ===
            # Pra cada GT/YOLO bbox, expandimos 50% e rejeitamos negativos com qq overlap.
            # Combina GT bboxes + bboxes YOLO (todas regiões possíveis de drone) → exclusion zones
            exclusion_bboxes = list(gt_bboxes_px)
            for pred in yolo_preds:
                exclusion_bboxes.append(tuple(pred["bbox_2d"]))
            # Expande cada exclusion 50% pra ter folga
            ...
            while added < n_negatives_to_add and tries < n_negatives_to_add * 10:
                ...
                # Rejeita se overlap ZERO+ com qq exclusion zone expandida
                overlap_any = False
...
def main():
    ...
    ap.add_argument("--band_m", type=float, default=0.0, help="depth-band (m): corta fundo distante do frustum")
    args = ap.parse_args()
