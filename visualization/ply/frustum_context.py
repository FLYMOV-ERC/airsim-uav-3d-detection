#!/usr/bin/env python3
"""
Write a PLY holding the WHOLE frame point cloud, the frustum points highlighted,
and the drone's 3D box. Useful for checking visually that the frustum crops
the right region.

Colours:
  - light grey: the whole cloud (outside the frustum)
  - yellow: the points inside the frustum
  - vermelho: wireframe bbox 3D drone
  - green: the drone centre
"""
import sys
import json
import argparse
...
def main():
    ...
    print(f"Writing {len(files)} PLYs with context:")
    for lbl_path in files:
        name = lbl_path.stem
        d = json.load(open(lbl_path))
        # Load the frame's original cloud (not the cropped frustum)
        ...
        if not full_pc_path.exists():
            print(f"  SKIP {name}: original point cloud not found")
            continue
        full_pc = np.load(full_pc_path).astype(np.float32)

        # Project each point: inside or outside the 2D box?
        ...
        # Colours: outside = dark grey, inside = yellow
        colors = np.zeros((len(full_pc), 3), dtype=np.uint8)
        colors[~inside] = [80, 80, 80]      # dark grey outside
        colors[inside] = [255, 230, 0]      # yellow inside the frustum
    ...
    print(f"\nOutput: {out_dir}")
    print(f"Colours: grey = scene, yellow = INSIDE the frustum, red = drone box, green = centre")
