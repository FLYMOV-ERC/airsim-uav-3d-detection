#!/usr/bin/env python3
"""
Convert frustum point clouds (.npy) to .ply for viewing in CloudCompare.
Includes the drone's 3D box as a red wireframe plus a green centre point.
...
def main():
    ...
    for npy_path in files:
        ...
        # Frustum points in GREY
        scene_colors = np.full((len(pc), 3), 150, dtype=np.uint8)
        all_pts = [pc]
        all_cols = [scene_colors]

        # Read the label to add the 3D box and its centre
