#!/usr/bin/env python3
"""PHASE 1 -- record a raw AirSim session (no inference, fast capture).

Saves per frame: RGB (jpg) + depth (npy float16) + x_plat (the ego's global pose) +
timestamp + the drones' global ground truth. evaluation/dump_detections.py then applies

The ego is HELD in the air and the targets fly by teleport (FlightThread).
"""
...
from dataset_generation.flight import FlightThread, _yaw_quat
from common.pipeline import airsim_pose_to_x_plat
...
def main():
    ...
    # Weather + time of day (reused from the generator)
    from dataset_generation.collect_airsim import apply_weather, apply_time_of_day
    ...
    # RANDOM positions within the range regime (azimuth +/-32 deg, to stay inside the FOV)
    ...
    for d in drones:
        ...
        gz = ego_g[2] + dist*math.tan(math.radians(15)) + rng.uniform(-3, 3)  # roughly centred for the -15 deg camera
    ...
    fly.start()
    print(f"[Rec] recording {args.duration}s (raw capture, no inference)...")
    ...
    try:
        while time.time() - t0 < args.duration:
            ...
            # ego global pose -> x_plat (ego held still: zero velocity)
            ego = obj_pose(cap, 'Ego')
            x_plat = airsim_pose_to_x_plat(ego, (0.0, 0.0, 0.0))
            # drones' global ground truth (for offline validation)
            gt = {}
            for d in drones:
                try:
    ...
    print(f"[Rec] {i} frames @ {fps:.1f} fps written to {out}/  (next: evaluation/dump_detections.py)")
