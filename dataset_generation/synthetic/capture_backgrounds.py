#!/usr/bin/env python3
"""Capture drone-free screenshots of the AirSim environment currently running.

Moves the target drones far away (out of the field of view) and captures RGB + depth
...
# Per-environment profile (reused from collect_airsim.py)
ENV_PROFILES = {
    "neighborhood": {
...
def capture(client):
    """Return (rgb_bgr, depth_array), or (None, None) on failure."""
...
def main():
    ...
    # Move the target drones VERY far away -- deep underground, so gravity cannot bring them back
    print("Moving the drones to (-5000, -5000, +2000) -- deep underground...")
    for v in vehicles:
        if v == "Ego":
            continue
        try:
    ...
    while n_saved < args.count and n_tried < args.count * 3:
        ...
        # Re-teleport the drones underground (gravity can bring them back)
        if n_saved % 10 == 0:
            for v in vehicles:
                if v == "Ego": continue
        ...
        # Random weather (every 20 captures)
        if n_saved % 20 == 0:
        ...
        # Capture (paused, to avoid a race)
        try:
        ...
        # Check for a collision (if the ego collided it may be inside a building)
