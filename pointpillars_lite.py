#!/usr/bin/env python3
"""PointPillars-lite (center-based BEV) — detector 3D voxel da CENA inteira, SEM YOLO.
Pillariza a nuvem (BEV X=fwd × Y=right), mini-PointNet por pillar → pseudo-imagem 2D →
CNN → heatmap de centros de drone + regressão (dx,dy,z,logsize). Paradigma alternativo ao frustum.
Dataset on-the-fly das sessões (depth→PC em CV + GT em CV). Treino do zero."""
...
# Grade BEV em coords CV: X=right [-40,40], Z=fwd [0,90]. células de 1.25m
import os as _os
    ...
    def __getitem__(self, idx):
        ...
        if TEMPORAL_K>0:   # acumula frames passados, compensando movimento do ego
            ...
            for dk in range(1,TEMPORAL_K+1):
                ...
                accs.append(_global_to_cv(g,xp)) # global → câmera ATUAL
            pc=np.concatenate(accs,axis=0).astype(np.float32)
        # pillariza: features por célula = [n, mean_xyz, max_xyz]... simplificado: ocupância+altura
...
def main():
    ...
    # split treino/val por sessão (80/20)
