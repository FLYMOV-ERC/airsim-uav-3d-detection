#!/usr/bin/env python3
"""Validação do EKFTracker3D em trajetória sintética.

Cenário:
  - Alvo com velocidade ~ constante + aceleração aleatória pequena
  - Plataforma móvel com pose RPY oscilante
  - Medições com ruído gaussiano no espaço esférico (d, phi, theta, r)
Critério: erro de posição em steady-state < 2m.
"""
import sys
import numpy as np

from ekf_3d_global import (
...
def simulate(n_steps: int = 200, dt: float = 0.1, seed: int = 42):
    ...
    for k in range(n_steps):
        ...
        plat[7, k] = np.radians(-15) + 0.05 * np.sin(0.2 * t)  # pitch (≈ -15° câmera)
        plat[8, k] = 0.1 * t                            # yaw
