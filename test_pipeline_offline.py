#!/usr/bin/env python3
"""Smoke-test do pipeline end-to-end OFFLINE (sem AirSim).

Carrega 1 frame do dataset, simula a thread de detecção+filtro:
  YOLO predictions (do disco) → PointNet++ → spherical → EKF
Renderiza overlay. Valida integração de todas as peças.
"""
import sys
import json
...
def main():
    ...
    # Reconstrói x_plat (apenas yaw — assume roll/pitch=0 do hover)
    ...
    # Filtro: simular 5 frames com mesma medida (estática) p/ confirmar tracks
    print("\n--- Filter (5 frames de medidas idênticas) ---")
    ...
    # Match bbox p/ track (heurística simples baseada em projeção)
    for t in tracks:
