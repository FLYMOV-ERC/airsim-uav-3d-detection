#!/usr/bin/env python3
"""Visualiza samples dos datasets painted_v5 e frustum_pn2_v2 como:
  - PLY (abrível em MeshLab/CloudCompare)
...
def make_bbox_3d_wireframe(center, size, color=(0, 255, 0), pts_per_edge=200,
                             min_size=0.05):
    """Wireframe 3D do AABB. Clampa size minimo pra ficar visível mesmo em bbox tiny."""
    cx, cy, cz = center
    # Clamp tamanho mínimo pra ficar visível
...
def make_center_sphere(center, color=(255, 0, 0), radius=0.04, n=400):
    """Esfera densa de pontos no centro do drone."""
    pts = []
    for _ in range(n):
        v = np.random.randn(3)
...
def main():
    ...
    ap.add_argument("--only_pos", action="store_true",
                    help="Só samples positivos (is_drone=1)")
    args = ap.parse_args()
    ...
    for pc_p, lbl_p, lbl in samples:
        ...
        xyz = pc[:, :3]
        # Cores: cinza por default; vermelho se prob>0 (painted)
        n = len(xyz)
        colors = np.full((n, 3), 180, dtype=np.uint8)  # cinza claro
        if pc.shape[1] >= 4:  # painted (xyz + prob)
            prob = pc[:, 3]
            inside = prob > 0
            colors[inside] = [220, 80, 80]   # red dentro do bbox YOLO
            colors[~inside] = [180, 180, 180]
        # Else (frustum): só XYZ, todos cinza
        ...
        sid = lbl_p.stem
        # Pinta de AZUL os pontos do PC próximos do center label (candidatos a drone)
        if center is not None:
            d = np.linalg.norm(xyz - center, axis=1)
            near = d < 0.15  # raio em frame normalizado
            colors[near] = [0, 100, 255]   # azul = pontos perto do label
        all_xyz = xyz
        all_colors = colors
        if center is not None:
