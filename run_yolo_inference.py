#!/usr/bin/env python3
"""
Roda YOLO11s treinado nos 3000 frames e salva predições por frame.

Output: <dataset>/yolo_predictions/<frame>.json com lista:
...
def main():
    ...
    ap.add_argument("--out_subdir", default="yolo_predictions",
                    help="Subdiretório de saída por dataset")
    args = ap.parse_args()
    ...
    for ds in args.datasets:
        if not Path(ds).exists():
            print(f"SKIP: {ds} não existe")
            continue
        nf, nd = process_dataset(model, ds, conf_threshold=args.conf, out_subdir=args.out_subdir)
        total_frames += nf
        total_det += nd

    print(f"\n{'='*50}\nTOTAL: {total_frames} frames com detecção, {total_det} detecções")
