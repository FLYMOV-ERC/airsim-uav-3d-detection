#!/usr/bin/env python3
"""Lightweight SORT (2D IoU + Hungarian) for tracking drones already confirmed by the PointNet.

- Association by the 2D box IoU (clean, from YOLO), not by the noisy 3D frustum position.
- A prior NMS collapses several boxes on the same object.
- Only already-confirmed detections (fused >= thr) enter, so tracks do not grow from noise.
- Each track carries the 3D information (range, centre) of its last associated measurement.
"""
import numpy as np
...
def nms(dets, iou_thr=0.4):
    """dets: list of dicts with 'bbox' and 'fused'. Keeps the highest-fused box per group."""
    if not dets: return []
...
class Track:
    ...
    def __init__(self, det):
        ...
        self.det = det                      # 3D information of the last measurement
...
class SortTracker:
    ...
    def confirmed(self):
        # includes tracks in a short COAST (prediction only) -> survives a 2D/3D dropout
