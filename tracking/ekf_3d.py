...
Spherical measurement (camera FRD frame, assuming camera = platform):
    y = [d, phi, theta, r] ∈ R^4
        d     = Euclidean range
        phi   = azimute  = atan2(y_c, x_c)
        theta = elevation = atan2(z_c, sqrt(x_c^2 + y_c^2))
...
# ─────────────────────────────────────────────────────────────────────────────
# Utilities (rotation, spherical/Cartesian conversions)
# ─────────────────────────────────────────────────────────────────────────────

def rotation_matrix(phi: float, theta: float, psi: float) -> np.ndarray:
    """R_yaw(psi) @ R_pitch(theta) @ R_roll(phi).  World -> camera rotation."""
...
def measurement_to_global(meas: np.ndarray, x_plat: np.ndarray) -> np.ndarray:
    """Spherical measurement + platform pose -> global position in the NED world frame."""
    d, phi, theta, _ = meas
...
@dataclass
class EKFParams:
    # Process model (random walk in acceleration)
    sigma_ax: float = 0.5
    sigma_ay: float = 0.5
    sigma_az: float = 0.5
...
class EKFTracker3D:
    """3D EKF for a single target with a spherical measurement."""
