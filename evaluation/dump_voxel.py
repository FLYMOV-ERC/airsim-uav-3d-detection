#!/usr/bin/env python3
"""Dump the PointPillars-lite detections (format compatible with evaluation/metrics.py).
Heatmap peaks -> 3D centre (CV) -> global position + projected 2D box. score = heatmap value."""
...
from detection.voxel.pointpillars import BEVDataset, PPNet, GH, GW, XMIN, ZMIN, CELL, TEMPORAL_K, _pc_to_global, _global_to_cv
...
def main():
    ...
    for s in sorted(glob.glob(args.sessions)):
        ...
        for fr in meta["frames"]:
            ...
            if TEMPORAL_K>0:   # accumulate past frames with ego compensation (as in training)
                accs=[pc]
            ...
            # pillarise (same logic as the dataset)
