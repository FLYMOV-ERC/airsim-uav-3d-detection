#!/usr/bin/env python3
"""Evaluation harness running the pipeline against the GROUND TRUTH of a recorded session.
Measures, per drone: YOLO recall (2D), post-fusion recall (after the 3D stage), and track-on-GT %.
Parameterised so hypotheses can be compared (conf, fusion, SAHI, max_age, depth band)."""
...
GT_PX = 60  # radius for matching a ground-truth target to a detection or track


def sahi_detect(yolo, bgr, conf, imgsz, nx, ny, overlap, full=True):
    """YOLO with tiling. Returns a list of (global_bbox, conf). Simple global NMS."""
    H, W = bgr.shape[:2]
    boxes = []
    if full:
...
def main():
    ...
    ap.add_argument("--det_band", type=float, default=0.0, help="depth band in the detector (match it to training)")
    args = ap.parse_args()
