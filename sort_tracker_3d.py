"""
3D SORT (Simple Online Realtime Tracking) for drone tracking.

Adapted from 2D SORT implementation used for person tracking.
State: [x, y, z, vx, vy, vz] in world NED coordinates.
Measurement: [x, y, z] 3D position from PointNet (or ground truth).

Uses:
- Constant-velocity Kalman filter per track
- Hungarian algorithm for detection-to-track association
- Euclidean distance cost metric
