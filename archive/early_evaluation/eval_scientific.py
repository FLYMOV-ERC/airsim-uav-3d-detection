#!/usr/bin/env python3
"""ARCHIVED. The pre-dump scientific evaluation over several sequences.

Metrics: detection (P/R/F1), 3D localization (RMSE/MAE stratified by range), and
MOT (MOTA, MOTP, IDF1, ID switches, MT/ML) via py-motmetrics, aggregated as
mean +/- standard deviation across sequences, comparing configurations.

This produced docs/results/scientific.md. Superseded by the dump protocol
(evaluation/dump_detections.py -> evaluation/metrics_ekf.py), which produces every
...
MATCH_GATE_M = 4.0   # a ground-truth/track pair matches when the 3D error is below 4 m
...
def process_seq(sess, yolo, det, cfg):
    ...
    pos_errs = []            # (gt_dist, err3d), for the RMSE stratified by range
    n_tp=n_fp=n_fn=0
    for fr in frames:
        ...
        # visible ground truth (projects inside the frame)
        ...
        # frame-level detection + 3D errors (greedy matching, for counting)
...
def main():
    ...
    # 3D error stratified by range
    e=np.array(all_errs)
    if len(e):
        print(f"  Global 3D error: RMSE={np.sqrt((e[:,1]**2).mean()):.2f} m  MAE={e[:,1].mean():.2f} m  (n={len(e)})")
