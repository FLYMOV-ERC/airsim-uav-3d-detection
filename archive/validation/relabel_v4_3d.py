#!/usr/bin/env python3
"""ARCHIVED. Recompute labels_3d for an already-collected dataset, using the manual box plus origin compensation.

The raw point clouds and metadata are kept; only labels_3d/*.json is replaced.
Output frame: CV camera (x=right, y=down, z=forward), the same as the raw clouds.

The migration tool that applied the central finding of Section 7.2.2 retroactively.
"""
import json
import math
import numpy as np
...
def in_fov(center_cv):
    """True when the projected centre falls inside both the horizontal and vertical FOV."""
