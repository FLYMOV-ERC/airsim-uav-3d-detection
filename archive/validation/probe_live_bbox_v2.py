#!/usr/bin/env python3
"""ARCHIVED. Live box check, second iteration: global poses plus the DroneVisualizer origin fix.

Writes a PLY with the cloud, the AirSim box and the manual box. Documents the
origin-frame fix that common/drone_visual.py now applies.
"""
...
from common.drone_visual import DroneVisualizer, vehicle_to_global
...
def main():
    ...
    # Raise the ego: global (0, 0, -25) with ego origin (0, 0, -5) means vehicle (0, 0, -20)
    ...
    # Attach VisQuad (with the origin fix)
    viz = DroneVisualizer(c, ['Drone3'], scale=4.0, sync_hz=30)
    viz.attach()
    viz.start_sync()
    time.sleep(1)

    # Place Drone3 (vehicle frame): its spawn origin is (15, -5, -5)
    # For a GLOBAL drone pose of (40, 0, -30), set vehicle (40-15, 0-(-5), -30-(-5)) = (25, 5, -25)
    ...
    # MANUAL computation from the true GLOBAL pose
    ...
    # The AirSim box arrives in the body frame (relative FRD). Convert it to CV:
    ...
    # Manual centre (blue) + MANUAL box using the Quadrotor1 4x extents
    sp, sc = make_sphere(manual_cv, (0, 100, 255))
    all_pts = np.vstack([all_pts, sp]); all_cols = np.vstack([all_cols, sc])
    # Quadrotor1 4x: body NED extents medidos via detection box3D (3.01, 3.93, 2.79)
    # 20% margin, to cover the spinning rotors that AirSim excludes from the AABB.
    ...
    # For each corner: world -> ego body (yaw=0) -> CV camera (pitch + lever arm)
