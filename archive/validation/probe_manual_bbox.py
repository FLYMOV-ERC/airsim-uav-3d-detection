#!/usr/bin/env python3
"""ARCHIVED. Validate the MANUAL 3D box built from the drone pose plus the known VisQuad 4x extents.

Compares it against the AirSim box on the SAME point cloud, so that the two can be
judged visually. The direct test of the central annotation finding of Section
7.2.2: the simulator's own box is rejected in favour of the geometric construction.
"""
import json
import numpy as np
...
def quat_to_rot_yaw_only(yaw_rad):
    """Approximation: the drone hovers with roll = pitch = 0."""
    c, s = np.cos(yaw_rad), np.sin(yaw_rad)
    return np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]])


def airsim_body_to_cv_camera(p_body):
    """Body NED (fwd, right, down) -> CV camera (right, down, fwd), applying the pitch and lever arm."""
...
def world_to_cv_camera(p_world, ego_pos, ego_yaw):
    ...
    # point relative to the ego body frame
...
def main():
    ...
    # five samples from each environment (frames that carry metadata)
    samples = []
    ...
    for env, ds, frame, meta, labels in samples[:10]:
        ...
        for l in labels:
            ...
            from visualization.raw_pc_with_bbox import airsim_to_cv, make_wireframe as mw_v
