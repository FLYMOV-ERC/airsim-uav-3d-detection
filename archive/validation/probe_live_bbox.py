#!/usr/bin/env python3
"""ARCHIVED. Rigorous live check of the 3D bounding-box construction.

1. Move the ego to a known pose (high, yaw=0, held against gravity)
2. Move Drone3 to exactly 15 m forward, 0 right, at the same altitude
3. Attach VisQuad_Drone3 at scale 4x
4. Capture the point cloud, the AirSim detection, and the manual construction
5. Write a PLY with the cloud, the AirSim box (red), the manual box (blue) and
   the raw drone pose as a point (yellow)

Part of the series that is the direct evidence for the central annotation finding
of Section 7.2.2.
"""
...
def main():
    ...
    # Re-set everything BEFORE the capture (gravity may have moved it)
    c.simSetVehiclePose(airsim.Pose(airsim.Vector3r(0, 0, EGO_Z), yq(0)), True, 'Ego')
    c.simSetVehiclePose(airsim.Pose(airsim.Vector3r(*DRONE_POS), yq(0)), True, 'Drone3')
    time.sleep(0.5)
    # Sync VisQuad pose
    ...
    if not dets:
        print("ERROR: VisQuad_Drone3 was not detected")
    ...
    # MANUAL COMPUTATION: the drone is at (15, 0, -30), the ego at (0, 0, -30)
    ...
    # ego at its current yaw -- use the full quaternion
    q = eq
    ...
    # subtract the camera offset (the camera sits at CAM_OFFSET_BODY in the body frame)
    ...
    print(f"AirSim cv (reorder FRD->CV only): ({rp.y_val:.2f}, {rp.z_val:.2f}, {rp.x_val:.2f})")

    # Write the PLY
    out = Path('viz_live')
    out.mkdir(exist_ok=True)
    # Cloud coloured by depth
    if len(pc) > 80000:
    ...
    # Manual box: uses the drone's true pose
