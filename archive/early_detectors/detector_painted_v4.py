#!/usr/bin/env python3
"""ARCHIVED. The PointPainting whole-cloud detector (PointNet++ Painted v4).

Pipeline (mirrors the painted dataset builder):
  1. Paint the full cloud with prob = YOLO confidence inside the box, 0 outside
  2. Resample to a fixed N=4096
  3. Normalise XYZ
  4. Forward pass through PointNet++
  5. Denormalise

Backs the "PointPainting (whole-cloud, 2D-painted)" paragraph of Section 7.3.4 --
the centre error falling from 3.6 to 0.003 and the validation F1 of 0.50 that
justify choosing the frustum crop instead. Provenance-critical.
"""
