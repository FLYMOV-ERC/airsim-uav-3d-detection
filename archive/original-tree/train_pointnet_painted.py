#!/usr/bin/env python3
"""
Treina PointNet PAINTED (PointPainting-style):
  - Input: (B, N=4096, 4) = xyz + prob_yolo
  - Output:
    * cls_logit: is_drone (handles YOLO false positives)
    * center: drone 3D center (regression, normalized)
    * size: drone 3D size (regression, normalized)

Class imbalance:
  - ~88% positives, ~12% negatives
  - Weighted BCE: peso maior nos negatives
  - Augmentation: rotação aleatória Y + jitter
"""
import json
import argparse
import numpy as np
...
class PaintedDroneDataset(Dataset):
    def __init__(self, root, split="train", augment=False):
        ...
        # Pre-load labels pra weighted sampling
        self.is_drone = []
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            # Rotação aleatória Y (mantém prob channel)
...
def main():
    ...
    ap.add_argument("--batch", type=int, default=16)  # N=4096 é pesado, batch menor
    ...
    # Weighted sampler para balancear classes
    ...
    # Sampler já balanceia → pos_weight=1 evita dupla-correção
    pos_weight = 1.0
