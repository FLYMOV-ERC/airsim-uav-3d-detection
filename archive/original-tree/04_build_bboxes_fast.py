...
def main():
    ...
    for cam in args.cameras:
        ...
        if not seg_dir.exists():
            print(f"⚠️  Skipping {cam}: segmentation directory not found")
            continue
        ...
        if not seg_files:
            print(f"⚠️  No segmentation files found in {seg_dir}")
            continue
        ...
        print(f"  ⏱️  Processed in {cam_time:.1f}s ({fps:.1f} frames/sec)")
