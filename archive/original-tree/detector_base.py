#!/usr/bin/env python3
"""Interface comum para os 3 métodos de estimação 3D pós-YOLO.

Cada método implementa:
    predict(pc, depth, bbox, conf) -> dict ou None
        return:
            'is_drone_prob': float (1.0 para depth direto)
            'is_drone': bool
            'center_cv': np.ndarray (3,) — posição cartesian no frame CV
            'size_cv': np.ndarray (3,) — dimensões CV
            'method': str (id do método para logging)
"""
