#!/usr/bin/env python3
"""
Constrói painted dataset (PointPainting-style) pra PointNet:
  ...
  - 1 detecção YOLO → 1 sample painted
  - Match GT por IoU 2D >= 0.3 pra atribuir label de regressão
  - Se YOLO detectou mas sem GT match: sample com cls=0 (false positive)
  - Negativos extras: frames sem detecção YOLO → todos zeros (cls=0)
"""
import json
import argparse
import numpy as np
...
def airsim_body_to_cv_camera(p_body):
    """body NED → CV camera (right, down, forward) com pitch + offset."""
...
def paint_pc(pc, yolo_bbox, confidence):
    """
    Para cada ponto, projeta pra pixel.
    Se dentro do bbox → prob = confidence
    Senão → prob = 0
    Retorna (N, 4) = xyz + prob
    """
...
def process_dataset(src_root, src_tag, out_root, rng, yolo_subdir="yolo_predictions"):
    ...
    n_pos = 0
    n_neg = 0
    n_fp = 0  # YOLO detectou sem GT match
    ...
    for split in ("train", "val"):
        ...
        for gt_yolo_p in sorted((gt_yolo_dir / split).glob("*.txt")):
            ...
            for k, pred in enumerate(yolo_preds):
                ...
                # Paint full PC com prob channel
                painted = paint_pc(pc, ybb, conf)

                # Sample fixo N=4096
                sampled = sample_to_fixed(painted, N_POINTS, rng)

                # Normaliza só XYZ (não toca prob)
                pc_xyz_norm, centroid, scale = normalize_pc(sampled[:, :3])
                sampled[:, :3] = pc_xyz_norm  # substitui xyz pelo normalizado

                # Label: se matched, regress; senão, false positive (cls=0)
                if best_iou >= 0.3 and best_gt:
                    # labels_3d v6 JÁ estão no frame CV camera. Usar direto.
            ...
            # === NEGATIVOS AGRESSIVOS: random crops em locais SEM drone ===
            # Pra cada YOLO det adicionada → 2 random negatives
            # Força balanço ~33%/67% pos/neg
            ...
            for neg_i in range(n_negatives_to_add):
                ...
                # Garante que não overlap com GT bbox (não é drone)
                max_iou_gt = max((bbox_iou(neg_bbox, gb) for gb in gt_bboxes_px), default=0)
                if max_iou_gt > 0.1:
                    continue
                # Random confidence (mimica falso positivo possível)
