#!/usr/bin/env python3
"""
Analisador de Sincronização LiDAR-Câmera
"""
...
def analyze_frame(frame_idx):
    ...
    if not all([rgb_path.exists(), fusion_path.exists(), metadata_path.exists(), lidar_path.exists()]):
        print(f"❌ Frame {frame_idx:04d} não encontrado completamente")
        return None
    ...
    # Analisa distribuição dos pontos LiDAR
    if len(points) > 0:
        ...
        if len(front_points) > 0:
            ...
            if len(anomaly_points) > 0:
                ...
                for i, pt in enumerate(anomaly_points[:5]):  # Mostra até 5 pontos
...
def visualize_sync_issue():
    """Visualiza o problema de sincronização"""
    print("Analisando problema de sincronização LiDAR-Câmera")
    print("="*60)

    # Analisa alguns frames específicos
