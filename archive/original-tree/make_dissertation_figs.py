#!/usr/bin/env python3
"""Generate publication-quality figures for the dissertation (Ch.4 pipeline hub, Ch.7 AirSim).
Vector PDFs written into ../dissertation/Cap4 and ../dissertation/Cap7. Quantitative figures use
the measured campaign results; qualitative figures use real session RGB/depth + detection dumps."""
import os, json, glob, math, numpy as np
import matplotlib
matplotlib.use("Agg")
...
CAP4 = "/home/ericyos/dissertation/Cap4"
CAP7 = "/home/ericyos/dissertation/Cap7"
os.makedirs(CAP4, exist_ok=True); os.makedirs(CAP7, exist_ok=True)
...
def fig_dataset_pipeline():
    ...
    try:
        sp = sorted(glob.glob("/home/ericyos/airsim/sprites/drone_*.png"))
    ...
    try:
        import matplotlib.image as mpimg
        bgs = sorted(glob.glob("/home/ericyos/airsim/backgrounds/*.jpg"))+sorted(glob.glob("/home/ericyos/airsim/backgrounds/**/*.jpg", recursive=True))
    ...
    try:
        import matplotlib.image as mpimg
        vis = sorted(glob.glob("/home/ericyos/airsim/dataset_synth/visualizations/*.jpg"))+\
              sorted(glob.glob("/home/ericyos/airsim/dataset_synth/yolo/images/train/*.jpg"))
...
def fig_depthband():
    sess = "/home/ericyos/airsim/sessions/nh_seq01_near_clear_h13"
    dump = json.load(open("/home/ericyos/airsim/det_dumps_segnet/nh_seq01_near_clear_h13.json"))
...
def fig_qualitative():
    ...
    for name in cands:
        dp=f"/home/ericyos/airsim/det_dumps_segnet/{name}.json"
    ...
    _,name,fr,kept=best
    sess=f"/home/ericyos/airsim/sessions/{name}"; i=fr["frame"]
...
# ----------------------------------------------------------------------------- Ch7: tracking stage
TRACKER_DUMP = "det_dumps_pfn"   # detector cujas detecções alimentam o tracker na Fig. 7.3b

def _run_tracker(name, thr=0.1):
    """Replica metrics_from_dump: roda o SORT num dump e devolve, por frame, o estado de cada track."""
    import importlib, sort_tracker as st
    importlib.reload(st)
    st.Track._next = 0
    trk = st.SortTracker(iou_thr=0.3, max_age=8, min_hits=3, nms_iou=0.4, coast=4)
    dump = json.load(open(f"/home/ericyos/airsim/{TRACKER_DUMP}/{name}.json"))
...
def fig_tracker():
    ...
    best = None
    for name in cands:
        if not os.path.exists(f"/home/ericyos/airsim/{TRACKER_DUMP}/{name}.json"): continue
        ...
        if ncoast < 2: continue                       # precisa mostrar algum coasting
        sc = coverage + ncoast                        # favorece cobrir bem o vídeo + ter coasting
        if best is None or sc > best[0]: best = (sc, name, rec, nF)
    if best is None:  # fallback: maior cobertura por tracks longos
        for name in cands:
            if not os.path.exists(f"/home/ericyos/airsim/{TRACKER_DUMP}/{name}.json"): continue
            rec, nF = _run_tracker(name)
    ...
    _, name, rec, nF = best
    lanes = {i: s for i, s in rec.items() if (s[-1][0]-s[0][0]) >= 20}
    order = sorted(lanes, key=lambda i: (lanes[i][-1][0]-lanes[i][0][0]), reverse=True)[:4]  # 4 mais longos
    order = sorted(order, key=lambda i: lanes[i][0][0])   # ordena por nascimento p/ exibir
