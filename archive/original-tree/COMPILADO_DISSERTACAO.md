# Compilado Técnico Completo — Detecção e Localização/Tracking 3D de Drones em Ambientes UAM Simulados (AirSim)
...
### 1.2 Desafios técnicos centrais
...
## 2. Roadmap do projeto (visão cronológica)
...
## 3. Infraestrutura de simulação e geometria
...
(msgpack) com o gateway WSL (ex.: `172.19.80.1:41451`).
...
### 3.4 Modelo de câmera (intrínsecos) — valores exatos
```
...
### 3.6 Profundidade → nuvem de pontos (CV)
```python
...
### 3.9 Sincronização e drift (`simPause` + re-teleporte) — por quê
...
### 4.1 Fase 1 — Dataset multimodal real (coleta no AirSim)
...
#### 4.1.1 Protocolo por frame (sincronização)
```
...
#### 4.1.3 Filtros de oclusão por profundidade (dois limiares)
...
#### 4.1.4 A limitação da API que disparou a virada de estratégia
...
### 4.2 O mesh visual ampliado (VisQuad 4×) — por quê e como
...
### 4.2.1 Correção crítica da bounding box 3D (achado central)
...
### 4.3 Fase 2 — A crise da rotulagem automática e a virada para o dataset sintético
...
#### 4.3.1 Tentativa 1 — Segmentação semântica (`ImageType.Segmentation`) — **falhou**
...
#### 4.3.2 Tentativa 2 — API de detecção (`simGetDetections`) — **falhou**
...
#### 4.3.3 Tentativa 3 — Projeção 3D→2D da pose conhecida — **falhou**
...
#### 4.3.4 Decisão: composição sintética (rótulos perfeitos, desacoplados da API)
...
### 4.4 Extração de sprites (`capture_sprites.py`)
...
### 4.5 Composição sintética (`compose_synthetic_dataset.py`)
...
### 4.6 Fase 3 — Campanha de avaliação (60 sequências de vídeo)
...
## 6. Detecção e localização 3D — evolução das arquiteturas
...
### 6.1 PointNet baseline (F1) — classificação frustum
...
### 6.2 PointPainting (F1) — canal de probabilidade do YOLO
...
### 6.4 Frustum PointNet++ (F2) — pipeline de 2 estágios consolidado
...
### 6.5 Depth-band — extração heurística de foreground (F2)
...
### 6.6 Normalização robusta p95 (F4) — resultado negativo
...
### 6.7 Frustum-ConvNet (F4) — fatias de profundidade
...
### 6.8 Segmentação T-Net / F-PointNet (F4) — substituto principled do depth-band
...
### 6.9 PointPillars-lite (F4) — paradigma voxel/BEV de 1 estágio
...
### 6.10 PointPillars-PFN (F5) — features de pillar aprendidas — **melhor localização**
...
### 6.11 Voxel 3D denso (F5, SECOND-style) — empata com o PFN
...
### 6.12 Variações voxel com resultado **negativo** (F5)
...
### 7.1 Fusão YOLO × PointNet (nível de score, F2)
...
### 7.2 Fusão tardia frustum ⊕ voxel (decision-level, F4–F5)
...
### 8.2 Tracker EKF 3D global (F1, histórico)
...
### 9.1 Métricas (padrão da literatura)
...
### 9.2 Protocolo de *dump* (eficiência + reprodutibilidade)
...
### 9.3 Caveat metodológico de rigor (a tratar na dissertação)
...
### 10.1 Configuração de referência do pipeline 2-estágios (best, 60 vídeos)
...
### 10.2 Ablação — depth-band + SORT vs baseline (frustum v4 max-norm + EKF)
...
### 10.3 Tabela-mestra — todos os métodos (60 vídeos, média 3 envs)
...
## 11. Achados científicos e lições
