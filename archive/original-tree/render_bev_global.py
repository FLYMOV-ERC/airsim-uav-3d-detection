#!/usr/bin/env python3
"""BEV TOP-DOWN em referencial GLOBAL FIXO (mundo). Mostra o ego se movendo + GT + tracker,
todos em coordenadas absolutas. Frame fixo dimensionado pelos limites da cena."""
...
def main():
    ...
    for dp in sorted(glob.glob(f"{args.dumps}/*.json")):
        ...
        def to_px(gx,gy):  # X(norte)=cima, Y(leste)=direita
        ...
        for fr in D:
            ...
            yaw=xp[8]; hx,hy=eu+int(14*np.cos(-yaw+np.pi/2))*0, ev  # marcador simples
