#!/usr/bin/env python3
"""Dump das detecções CRUAS (YOLO+frustum) por frame, por vídeo — UMA passada pesada.
Salva: por frame, lista de detecções [bbox, yolo_conf, pn_prob, gpos(3D global)] + GT.
Depois metrics_from_dump.py calcula CLEAR-MOT/AMOTA/AP variando threshold (barato)."""
...
def main():
    ...
    ap.add_argument("--conf", type=float, default=0.05)  # baixo p/ varrer depois
    ...
    for s in sessions:
        ...
        for fr in meta["frames"]:
            ...
            # caixas YOLO: usa cache se houver, senão roda YOLO e salva
