#!/usr/bin/env python3
"""
Treina PointNet pra detecção de drone:
  - Input: (N=512, 3) point cloud
  - Output:
    * cls_logit (B, 1): binary "is_drone"
    * center (B, 3): drone center NORMALIZED (relative to PC centroid, /scale)
...
class FrustumDroneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pc = np.load(pc_path).astype(np.float32)  # (N, 3) já normalizado
        ...
        # Augmentação simples: random rotação Y, jitter
        if self.augment:
...
class PointNetDetector(nn.Module):
    def __init__(self):
        super().__init__()
        # Shared MLPs (Conv1d são equivalentes a MLP point-wise)
