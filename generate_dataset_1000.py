#!/usr/bin/env python3
"""
DATASET GENERATOR - 1000+ FRAMES
Gera dataset grande com muitas variações para YOLO e PointNet
"""
...
# Visualizações (só algumas para não ocupar muito espaço)
...
detection_radius_cm = 15000  # 150 metros para detectar mais longe
...
# MUITOS MAIS CENÁRIOS E VARIAÇÕES
scenarios = []

# Cenários de distância variada
...
# Muitos ângulos de yaw para cada cenário
yaw_angles = list(range(-45, 46, 3))  # -45 a 45 graus, passo de 3 graus = 31 ângulos

total_frames = 0
max_frames = 1500  # Meta de 1500 frames para garantir 1000+ com labels
...
# Embaralha cenários para variedade
...
for scenario_idx, scenario in enumerate(scenarios):
    ...
    if scenario_idx % 10 == 0:  # Mostra progresso a cada 10 cenários
        print(f"\n🎬 Cenário {scenario_idx+1}/{len(scenarios)}: {scenario['name'][:30]}")
        print(f"   Progresso total: {total_frames}/{max_frames} frames")
        print(f"   Com detecções: {frames_with_detections}, Sem detecções: {frames_without_detections}")
    ...
    time.sleep(1)  # Tempo reduzido para acelerar

    # Seleciona subset de ângulos aleatoriamente para este cenário
    ...
    for yaw in angles_for_scenario:
        ...
        if img_bgr is not None and depth_array is not None:
            ...
            if detections:
                ...
                # Cria visualização apenas para alguns frames
                vis = None
                if total_frames % 50 == 0:  # Visualização a cada 50 frames
                    vis = img_bgr.copy()
                ...
                for det in detections:
                    if hasattr(det, 'box2D'):
                        try:
                            ...
                            if total_frames % 100 == 0:  # Mostra erro ocasionalmente
                                print(f"   ⚠️ Erro processando detecção: {e}")

                # Salva apenas se tem labels
                if yolo_labels:
                    # Salva imagem RGB
                    ...
                    # Salva visualização (apenas alguns frames)
                    if vis is not None:
                        cv2.imwrite(str(vis_dir / f"{frame_name}.jpg"), vis)

                    # Metadata (apenas para alguns frames para economizar espaço)
            ...
            if total_frames % 100 == 0:
                ...
                print(f"      Com detecções: {frames_with_detections}")
                print(f"      Taxa de sucesso: {frames_with_detections/total_frames*100:.1f}%")
...
print(f"   • Frames com detecções: {frames_with_detections}")
print(f"   • Taxa de sucesso: {frames_with_detections/total_frames*100:.1f}%")
print(f"   • Imagens de treino: {num_train}")
print(f"   • Imagens de validação: {num_val}")
print(f"   • Total com labels: {num_train + num_val}")
print(f"\n📁 Estrutura:")
print(f"   {output_dir}/")
print(f"   ├── yolo/")
print(f"   │   ├── images/train/ ({num_train} imagens)")
print(f"   │   ├── images/val/ ({num_val} imagens)")
print(f"   │   ├── labels/train/")
print(f"   │   ├── labels/val/")
print(f"   │   └── dataset.yaml")
print(f"   ├── pointnet/")
print(f"   │   ├── point_clouds/ ({num_train + num_val} arquivos .npy)")
print(f"   │   └── labels_3d/")
print(f"   └── visualizations/ (amostras)")
print(f"\n✅ Dataset pronto para treinar YOLO e PointNet!")