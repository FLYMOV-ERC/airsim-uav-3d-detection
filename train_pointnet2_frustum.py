#!/usr/bin/env python3
"""Treina PointNet++ no dataset frustum-YOLO (cada bbox YOLO @conf=0.10 é 1 sample).

Input: (B, N=4096, 3) = xyz só (sem prob channel — frustum já é cropado)
Output:
  - cls_logit (B,): is_drone (filtra YOLO false positives)
  - center (B, 3): drone 3D center normalizado
  - size (B, 3): drone 3D size normalizado

Treino com Focal Loss + WeightedRandomSampler 50/50.
"""
import json
import argparse
import numpy as np
...
from train_pointnet2_painted import (
...
def main():
    ...
    # PointNet++ com in_channels=3 (sem prob)
    if args.arch == "convnet":
        from frustum_convnet import FrustumConvNet
    ...
    # Warm-start cruzado: carrega pesos de um checkpoint (ex: painted) que casam
    # por nome+shape. Só sa1.mlp_convs.0.weight difere (3ch vs 4ch) e fica fresh.
    if args.warm_start:
