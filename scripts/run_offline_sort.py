#!/usr/bin/env python3
"""Process a recorded session with YOLO at 1280 -> frustum PointNet -> fusion -> NMS -> SORT (2D IoU).
Tracks come ONLY from confirmed detections (fused >= thr); NMS deduplicates; SORT associates by IoU.
"""
...
def main():
    ...
    ap.add_argument("--max_dist", type=float, default=90.0, help="reject a 3D measurement beyond this (background noise)")
    ...
    for fr in frames:
        ...
        if res and res[0].boxes is not None and len(res[0].boxes) > 0:
            ...
            for k in range(len(xyxy)):
                ...
                if fused < args.fuse_thr: continue          # only those confirmed by the PointNet fusion
                if d3 > args.max_dist: continue              # reject an implausible 3D range (background)
                dets.append({'bbox': bb, 'fused': fused, 'd': d3,
                             'yolo_conf': conf, 'pn_prob': pn})
        tracks = tracker.update(dets)
        # overlay
        vis = bgr.copy()
        for d in dets:  # confirmed boxes (before tracking) drawn as thin grey
        ...
        # projected ground truth -- a yellow cross, showing the track matches the real drone
