#!/usr/bin/env python3
"""Top-down (bird's-eye) view of the ground truth against the tracker, in global coordinates.
Processes a session with the best configuration; for each track the 3D centre (camera frame)
is converted to global coordinates and drawn on a map seen from above: the ego, the true
targets (filled circles) against the tracker estimates (hollow squares), with trails.
"""
...
def main():
    ...
    # BEV canvas parameters (metres -> pixels). X = forward (up), Y = right (right)
    W = H = 760; SCALE = 7.0  # px per metre
    OX, OY = W//2, H - 80      # ego near the bottom, horizontally centred

    def to_px(gx, gy, ego):
        # relative to the ego in a global FRD-like frame: X (forward/north), Y (right/east) of global NED
    ...
    all_gt = {n:[] for n in GT_COL}; all_trk = {}  # for the static plot
    ...
    for fr in frames:
        ...
        if r and r[0].boxes is not None:
            for k in range(len(r[0].boxes.xyxy)):
                ...
                # global centre: ego + R(rpy) @ FRD(center_cv)
        ...
        # ego (triangle pointing up = forward)
        ...
        # ground truth (filled circle + trail)
        for n,g in gt.items():
        ...
        # tracker (hollow square + trail) + error against the nearest ground truth
        for t in tracks:
            ...
            # error to the nearest ground truth (global 3D)
            best=1e9; bn=None
    ...
    print("Global 3D tracker-vs-ground-truth error (m):")
