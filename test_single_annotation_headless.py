...
def process_single_image(rgb_path, seg_path, color_map_path, output_dir):
    ...
    # Load images
    print(f"\n📸 Processing image: {rgb_path.name}")
    ...
    for (b, g, r), cls in color_to_class.items():
        ...
        for i, contour in enumerate(contours):
            ...
            draw_bbox(vis_rgb, x, y, w, h, f"{cls}", class_color)
            ...
            print(f"      Detection {valid_detections}: bbox=[{x},{y},{w},{h}], area={area} pixels")
    ...
    with open(yolo_file, "w") as f:
        for ann in yolo_annotations:
            ...
            print(f"\n   YOLO: {class_id} {ann['cx']:.6f} {ann['cy']:.6f} {ann['w']:.6f} {ann['h']:.6f}")
    ...
    print(f"\n💡 View the comparison.png file to see the results!")
...
def main():
    ...
    if not rgb_path.exists():
        ...
        img_dir = Path(args.dataset) / "images" / args.camera
        if img_dir.exists():
            frames = sorted(img_dir.glob("*.png"))[:5]
            if frames:
                print(f"\n   Available frames:")
                for f in frames:
                    print(f"      {f.name}")
    ...
    if not Path(args.seg_map).exists():
        ...
        print("\n   Run first:")
        print("   python3 03_map_seg_colors_auto.py --seg_dir dataset/seg/front_center --classes DRONE")
        return
