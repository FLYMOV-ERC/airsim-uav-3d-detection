#!/usr/bin/env python3
"""
Frustum-PointNet dataset builder.

Pra cada (frame, bbox_2d), projeta os pontos do PC pra image space e mantém
SÓ os dentro do bbox. Cada bbox vira UM sample do frustum dataset.

Frame com 3 bboxes → 3 frustum samples.

Frame de coords:
  PC do v17 está em CV camera frame: X=right, Y=down, Z=forward.
  Projeção: u = X*FX/Z + CX,  v = Y*FY/Z + CY  (mesma do depth_to_pointcloud).
...
Output:
    ...
    <output>/viz/<src>_frame_XXXXXX_obj<i>.jpg            (opcional)
"""
import sys
import json
import argparse
...
FY = FX  # square pixels (igual ao generator)
...
def crop_frustum(pc, bbox_2d, margin_px=2):
    """
    Filtra pontos do PC (CV frame) que projetam dentro do bbox 2D.
    Retorna subset Nx3.
    """
    x1, y1, x2, y2 = bbox_2d
    z = pc[:, 2]
...
def render_viz(image, bbox_2d, label, n_points, dist):
    """Imagem original com bbox destacado + info do frustum."""
    out = image.copy()
    x1, y1, x2, y2 = bbox_2d
...
def process_dataset(root, out_dir, source_tag, viz_limit=20):
    """Processa um dataset (city/nh/coast) e adiciona ao output."""
    root = Path(root)
    ...
    for split in ("train", "val"):
        for img_path in sorted((img_dir / split).glob("*.jpg")):
            ...
            # Carrega imagem só se for fazer viz
            image = None
            ...
            for i, line in enumerate(yolo_lines):
                ...
                # Acha label 3D correspondente (match por bbox)
                matched = None
                for d in labels_3d:
                    bb = d.get("bbox_2d")
                    if not bb: continue
                ...
                if matched:
                    # Converte center/box3D AirSim → CV (alinhado com PC)
...
def main():
    ...
    ap.add_argument("--output", default="dataset_frustum",
                    help="Dir de saída (default: dataset_frustum)")
    ...
    ap.add_argument("--viz", type=int, default=20,
                    help="Qntas viz salvar POR dataset (default: 20)")
    args = ap.parse_args()
    ...
    print(f"  ({total_empty} skips por crop vazio)")
