...
Implementação from-scratch em PyTorch puro:
...
def square_distance(src, dst):
    """
    Squared distance entre cada par (B, N, C) e (B, M, C).
    Retorna (B, N, M).
    """
    B, N, _ = src.shape
    _, M, _ = dst.shape
...
def farthest_point_sample(xyz, npoint):
    """
    FPS: seleciona npoint pontos que ficam o mais longe possível uns dos outros.
    xyz: (B, N, 3)
    return: idx (B, npoint)
    """
    device = xyz.device
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    """
    Ball query: pra cada centroide em new_xyz, retorna idx dos nsample pontos
    mais próximos dentro do raio.
...
def sample_and_group_all(xyz, points):
    """Para a última SA layer: trata todos os pontos como 1 grupo."""
    device = xyz.device
    B, N, C = xyz.shape
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all=False):
        ...
        nsample: pontos por centroid
        in_channel: features de entrada (ex 4 = xyz_norm + prob, ou 3 + feat_dim)
...
class PointNet2Painted(nn.Module):
    def __init__(self, in_channels=4):
        ...
        in_channels: total channels do input (xyz + extras).
          4 = xyz + prob (painted)
          3 = xyz only (frustum sem prob channel)
        """
        super().__init__()
    ...
    def forward(self, x):
        """
        x: (B, N, C) onde C=4 (painted: xyz+prob) ou C=3 (xyz only)
        """
        xyz = x[:, :, :3]
        if x.size(-1) > 3:
...
# =============================================================================
# Dataset (mesma do train_pointnet_painted.py)
# =============================================================================
...
def main():
    ...
    ap.add_argument("--batch", type=int, default=8)  # PointNet++ é mais pesado
    ...
    ap.add_argument("--warm_start", default="", help="checkpoint .pt pra warm-start cruzado")
    args = ap.parse_args()
    ...
    # Sampler balanceia 50/50 + focal alpha=0.5 (neutro nos batches balanceados)
    ...
    # Warm-start cruzado: carrega pesos casando por nome+shape (ex: do frustum).
    ...
    for epoch in range(1, args.epochs+1):
        ...
        # Save best por F1 (não acc — já que classes desbalanceadas)
