#!/usr/bin/env python3
"""Dense 3D voxel backbone (VoxelNet/SECOND style, WITHOUT spconv) -- the full 3D paradigm.
Voxelises the cloud into a 3D grid (X=right, Y=down, Z=forward), a Conv3D backbone, then collapses the
height (down) dimension -> BEV pseudo-image -> center-based head (heatmap + regression).
Unlike PointPillars, which collapses the height immediately, here the height is processed
by 3D convolutions before collapsing. Modes: train (default) and dump (--dump)."""
...
# 3D grid in CV coordinates. 2.0 m cells (dense 3D is expensive, so a coarser grid than PointPillars)
...
class Vox3DNet(nn.Module):
    def __init__(self):
        super().__init__()
        # 3D backbone: reduce the height (D) by stride, keep H and W
    ...
    def forward(self,x):
        ...
        B,C,d,H,W=f.shape
        f=f.reshape(B,C*d,H,W)                  # collapse the height
...
def dump(args):
    ...
    ds=Vox3DDataset([])  # only to reuse voxelize
