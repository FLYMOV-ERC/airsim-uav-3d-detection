#!/usr/bin/env python3
"""Train FrustumSegNet: cls + centre + size + foreground/background segmentation, with the
per-point labels derived on the fly (points inside the normalised ground-truth box). Same frustum dataset (v4)."""
...
from detection.frustum.train import FrustumPN2Dataset
from detection.pointnet.pointnet2 import FocalLoss
from detection.frustum.segnet import FrustumSegNet
import torch.nn.functional as F


def fg_labels(pc, c_gt, s_gt):
    # pc:(B,N,3) c_gt:(B,3) s_gt:(B,3) -> (B,N), 1 where the point falls inside the box
