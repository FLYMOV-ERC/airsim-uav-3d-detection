#!/usr/bin/env python3
"""MultiEKFTracker — gerencia múltiplos EKFTracker3D.

Loop por frame:
  1. predict() em cada track
  2. Associação Hungarian no espaço de medição (Mahalanobis com gating χ²)
  3. Update de tracks matched
  4. Cria novo track p/ unmatched detections
  5. Lifecycle (max_age, min_hits)
  6. Retorna tracks confirmados
"""
...
from ekf_3d_global import EKFTracker3D, EKFParams, wrap_angle
...
class MultiEKFTracker:
    ...
    def update(self,
        ...
        """
        measurements: lista de tuples/listas onde os 4 primeiros valores
                       são (d, phi, theta, r). Opcionalmente (..., conf, ...).
        x_plat: (9,) estado da plataforma
        dt: tempo (s) desde último update
        return: lista de tracks confirmados (referências aos objetos)
        """
        self.frame_count += 1
        ...
        # Cria novos tracks para detecções unmatched
